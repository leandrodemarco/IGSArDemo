...
//  ARHoops
//
//  Created by Leandro Demarco Vedelago on 22/8/17.
//  Copyright © 2017 Gradient Labs LLC. All rights reserved.
//
...
//        return normalized
        ...
        // !!! need to be tested and replace code above
        return ref + (angle - ref).truncatingRemainder(dividingBy: .pi / 2)
    }
    ...
    private lazy var fillPlane: SCNNode = {
        let c = self.focusSquareThickness / 2 // correction to align lines perfectly
        let plane = SCNPlane(width: CGFloat(1.0 - self.focusSquareThickness * 2 + c),
                             height: CGFloat(1.0 - self.focusSquareThickness * 2 + c))
    ...
    private lazy var focusSquareNode: SCNNode = {
        ...
        let c: Float = self.focusSquareThickness / 2 // correction to align lines perfectly
        ...
        planeNode.addChildNode(self.fillPlane)
